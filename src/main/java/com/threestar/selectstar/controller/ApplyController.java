...
public class ApplyController {
    ...
    @GetMapping("/check")
    public ResponseEntity<ApplyCheckResponse> checkByUserIdAndMeetingId(@RequestParam int userId, @RequestParam int meetingId){
        try {
            return ResponseEntity.ok()
                    .body(applyService.checkApply(userId,meetingId)); // 신청 없으면 에러
    ...
    @PatchMapping("/reject")
    public Map<String,String> rejectApply(@RequestBody RejectApplyRequest rejectApplyRequest){
        Map<String, String> succesMap = new HashMap<>();
